...
use cuda_tile_rs::{
    cuda_tile, cuda_tile_write_bytecode, cuda_tile_write_bytecode_to_buffer,
    register_cuda_tile_dialects,
};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Try to translate to bytecode.
    let string_ref = cuda_tile_write_bytecode_to_buffer(&module_op);
    println!("bytecode length: {:#?}", string_ref.to_raw().length);

    // Try to write bytecode.
    let bc_filename = concat!(env!("OUT_DIR"), "/__basic_kernel.bc");
    let res = cuda_tile_write_bytecode(&module_op, bc_filename);
    assert!(res.is_ok());
    let cubin_filename = concat!(env!("OUT_DIR"), "/__basic_kernel.cubin");
